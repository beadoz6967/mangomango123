...
void Visuals::init() {
    uintptr_t scanViewMatrix = M::patternScan("client", "48 8D 0D ? ? ? ? 48 C1 E0 06");
    if (scanViewMatrix) {
        viewMatrix.viewMatrix = (viewmatrix_t*)M::getAbsoluteAddress(scanViewMatrix, 3, 0);
    } else {
        viewMatrix.viewMatrix = nullptr;
    }
    Logger::Logf(LogType::Info, "scanViewMatrix: %s | ptr=%p", scanViewMatrix ? "OK" : "FAILED", viewMatrix.viewMatrix);
}

void Esp::cache()
{
    static int s_cacheCallCount = 0;
    bool dbg = (++s_cacheCallCount % 300 == 1); // log every ~300 frames

    if (!I::EngineClient || !I::EngineClient->valid()) {
        if (dbg) Logger::Log("cache: EngineClient invalid", LogType::Warning);
        return;
    }
    if (!I::GameEntity || !I::GameEntity->Instance) {
        if (dbg) Logger::Log("cache: GameEntity/Instance null", LogType::Warning);
        return;
    }
    ...
    * @ // @here: manually cache once all existing entitys
    ...
    if (dbg) Logger::Logf(LogType::Info, "cache: highestIdx=%d", nMaxHighestEntity);
    ...
    if (dbg) Logger::Logf(LogType::Info, "cache: cached_players=%zu localAlive=%d", cached_players.size(), (int)cached_local.alive);
}

void Visuals::esp() {
    static int s_espCallCount = 0;
    bool dbg = (++s_espCallCount % 300 == 1);

    if (!Config::esp && !Config::showHealth && !Config::espFill && !Config::showNameTags) {
        if (dbg) Logger::Log("esp: all config false, returning", LogType::Warning);
        return;
    }

    if (!H::oGetLocalPlayer) {
        if (dbg) Logger::Log("esp: oGetLocalPlayer null", LogType::Warning);
        return;
    }

    C_CSPlayerPawn* localPawn = H::oGetLocalPlayer(0);
    if (!localPawn) {
        if (dbg) Logger::Log("esp: localPawn null (not in game?)", LogType::Warning);
        return;
    }

    if (cached_players.empty()) {
        if (dbg) Logger::Log("esp: cached_players empty", LogType::Warning);
        return;
    }
